...
class LegacyForgePlugin implements Plugin<Project> {
        ...
        proj.with {
            ...
            if (hasCurseforge) {
                curseforge {
                    ...
                    it.project {
                        id = "${propertyWithFallback(proj, "curseforge_id", "curseforge_id_forge")}"
                        ...
                        var dependencies = proj.curseforge_dep_forge
                        if (!"${dependencies}".isEmpty() || proj.hasProperty("optional_curseforge_dep_forge")) {
                            relations {
                                ...
                                if (proj.hasProperty("optional_curseforge_dep_forge")) {
                                    "${optional_curseforge_dep_forge}".split(", ").each {
            ...
            if (hasModrinth) {
                modrinth {
                    ...
                    loaders = ["forge"]
                    dependencies {
                        var dependencies = proj.modrinth_dep_forge
                        if (!"${dependencies}".isEmpty()) {
                            "${dependencies}".split(", ").each {
                                required.project "${it}"
                            }
                        }
                        if (proj.hasProperty("optional_modrinth_dep_forge")) {
                            "${proj.optional_modrinth_dep_forge}".split(", ").each {
                                optional.project "${it}"
                            }
                        }
                    }
